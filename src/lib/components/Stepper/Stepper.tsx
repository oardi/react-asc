...
export const Stepper = (props: IStepperProps): React.JSX.Element => {
	const {
		children,
		isLinear = true,
		showLabel = true,
	...
	const renderSteps = (child: React.ReactNode, index: number): React.JSX.Element => {
		return (
			<>
					...
					{steps &&
						steps.map((step, index) => (
							<Fragment key={step.props.value}>
								{index === activeIndex && <StepPanel>{step.props.children}</StepPanel>}
							</Fragment>
						))}
